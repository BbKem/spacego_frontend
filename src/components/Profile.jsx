...
function Profile({ user, onBack, setCurrentPage }) {
  ...
  // Функции фильтрации без логирования
  const isArchived = (ad) => {
  ...
  const isActive = (ad) => {
    return ad.status === 'approved' && !isArchived(ad);
  };
  
  const isPending = (ad) => {
    return ad.status === 'pending' && !isArchived(ad);
  };
  
  const isRejected = (ad) => {
    return ad.status === 'rejected' && !isArchived(ad);
  };
  ...
  const getAdsCountByStatus = () => {
    return {
      active: userAds.filter(isActive).length,
      pending: userAds.filter(isPending).length,
      archived: userAds.filter(isArchived).length,
      rejected: userAds.filter(isRejected).length
    };
  };
  ...
  const handleAdClick = (ad) => {
    ...
    setCurrentPage('view-ad');
    localStorage.setItem('selected_ad', JSON.stringify(ad));
  };
